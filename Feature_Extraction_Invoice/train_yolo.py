...
Examples:
  python3 Feature_Extraction_Invoice/train_yolo.py --prepare-only
  python3 Feature_Extraction_Invoice/train_yolo.py --epochs 100 --model yolov8n.pt
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--project", default=str(ROOT / "runs"), help="Ultralytics project directory")
    ...
    if YOLO_DIR.exists():
        if not args.force:
            raise FileExistsError(
                f"{YOLO_DIR} already exists. Re-run with --force to rebuild it."
            )
